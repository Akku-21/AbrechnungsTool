...
export default function SettlementDetailPage({
    ...
    useEffect(() => {
        if (id) {
            invoicesApi
                .getDefaultAllocation(id)
                .then((data) => {
                    setDefaultAllocation(data.default_allocation);
                    setInvoiceForm((prev) => ({
                        ...prev,
                        allocation_percentage: data.default_allocation,
                    }));
                })
                .catch(console.error);
        }
    ...
    const handleDeleteSettlement = async () => {
        const confirmed = await confirm({
            ...
            message:
                "Abrechnung wirklich löschen? Alle zugehörigen Dokumente und Rechnungen werden ebenfalls gelöscht.",
            confirmLabel: "Löschen",
            variant: "destructive",
        });
        if (confirmed) {
    ...
    const statusConfig =
        STATUS_CONFIG[settlement.status] || STATUS_CONFIG.DRAFT;
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            ...
                            <span
                                className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-sm font-medium ${statusConfig.color}`}
                            >
                ...
                <nav className="flex gap-2 overflow-x-auto">
                    ...
                    >
                        ...
                        {documents && documents.length > 0 && (
                            <span className="ml-1 px-1.5 py-0.5 text-xs bg-gray-200 rounded-full">{documents.length}</span>
                        )}
                    </button>
                    <button
                    ...
                    >
                        ...
                        {invoices && invoices.length > 0 && (
                            <span className="ml-1 px-1.5 py-0.5 text-xs bg-gray-200 rounded-full">{invoices.length}</span>
                        )}
                    </button>
                    {invoices && invoices.length > 0 && (
                        <button
                            onClick={() => document.getElementById('unit-settlements-section')?.scrollIntoView({ behavior: 'smooth' })}
                            className="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-600 hover:text-gray-900 hover:bg-white rounded-md transition-colors whitespace-nowrap"
                        >
            ...
            <Card id="documents-section" className="scroll-mt-16">
                <CardHeader className="flex flex-row items-center justify-between">
                    <div>
                        ...
                        <CardDescription>
                            Hochgeladene Belege und Rechnungsdokumente
                        </CardDescription>
                    </div>
                    <div>
                        <input
            ...
            <Card id="invoices-section" className="scroll-mt-16">
                <CardHeader className="flex flex-row items-center justify-between">
                    <div>
                        ...
                        <CardDescription>
                            Erfasste Rechnungen für diese Abrechnung
                        </CardDescription>
                    </div>
                ...
                <CardContent>
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        Anteil der Kosten, der auf Mieter
                                        umgelegt wird
                                    </p>
                                </div>
                            </div>
                    ...
                    {invoicesLoading ? (
                        ...
                        <div className="text-center py-8">
                            ...
                            <p className="mt-1 text-sm text-gray-500">
                                Fügen Sie Rechnungen hinzu oder verarbeiten Sie
                                Dokumente per OCR.
                            </p>
                        </div>
                    ) : (
            ...
            {invoices && invoices.length > 0 && (
                <div id="unit-settlements-section" className="scroll-mt-16">
                    <UnitSettlementsList
                        settlementId={id}
                    ...
                    <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
                        <div className="p-6">
                            ...
                            <p className="text-gray-600 mb-4">
                                Nach der Finalisierung kann diese Abrechnung
                                nicht mehr bearbeitet oder gelöscht werden.
                            </p>
                            <p className="text-gray-600 mb-6">
                                Falls später Änderungen nötig sind, können Sie
                                eine <strong>Korrekturabrechnung</strong>{" "}
                                erstellen.
                            </p>
